...
with open('archive(1)/train.fr', 'r') as corpus:
    for line in corpus:
        words = line.split()
        for word in words:
...
for sentence_ids in all_token_ids:
    if len(sentence_ids) > 49:
        print("Found oversized sentence!")

    while len(sentence_ids) < 49:
        sentence_ids.append(token_id_dict["<PAD>"])

tensor = torch.tensor(all_token_ids)
torch.save(tensor, "train_fr_ids.pt")
torch.save(token_id_dict, 'token_info_dict_fr.pt')
